<template>
  <section class="relative overflow-hidden border-b border-[#e8e6de] bg-[#fdfcf8] supports-[backdrop-filter]:bg-white/85 supports-[backdrop-filter]:backdrop-blur-[8px]">
    ...
    <div class="relative container mx-auto max-w-[1280px] px-6 py-8">
      <!-- main row: eyebrow + title + desc + stats -->
      <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 lg:gap-8">
        ...
        <div class="max-w-[44rem]">
          ...
          <div class="inline-flex flex-wrap items-center gap-2 mb-4">
            ...
            <span class="text-[11px] tracking-[0.2em] font-semibold text-emerald-800 uppercase">{{ eyebrow }}</span>
            <span v-if="eyebrowAccent" class="hidden sm:inline-flex items-center gap-2">
              <span class="w-1 h-1 rounded-full bg-amber-400"></span>
              <span class="text-[11px] tracking-[0.2em] font-medium text-stone-500 uppercase">{{ eyebrowAccent }}</span>
            </span>
          </div>
          ...
          <p class="mt-3.5 text-sm sm:text-[16px] leading-relaxed text-stone-600 max-w-[38rem] text-pretty">
            {{ description }}
          </p>
        </div>

        <!-- right: stats (desktop) -->
        <div v-if="$slots['stats-right']" class="hidden lg:flex flex-col items-end text-right shrink-0">
          <slot name="stats-right" />
        </div>
      </div>

      <!-- bottom bar: pills left / primary actions right — unified divider h-px bg-[#e8e6de] -->
      <div
        v-if="$slots['actions-left'] || $slots['actions-right']"
        class="mt-7 pt-5 border-t border-[#e8e6de] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
      >
